...
class PostgreSQLConnection extends SQLConnectionBase {
  ...
  getDefaultFieldValue(type) {
    switch (type) {
      case 'AUTO_INCREMENT':
        ...
        return new Literals.Literal('@@@AUTO_INCREMENT@@@', { noDefaultStatementOnCreateTable: true, remote: true });
      case 'DATETIME_NOW':
  ...
  async query(_sql, _options) {
    ...
    try {
      client = options.transaction || this.inTransaction;
      if (!client)
  ...
  _bigintTypeToString(type, _options) {
    ...
    if (options.createTable && options.defaultValue === '@@@AUTO_INCREMENT@@@')
